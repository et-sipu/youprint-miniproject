...
class UploadCSV implements ShouldQueue
{
    ...
    {
        try {
            // Update process status into Processing
            $uploadedFiles = UploadedFiles::where('id',$this->file_id);
            $uploadedFiles->update(['status' => 1]);
            ...
            foreach ($csv as $record){
                ...
                //Check if there's a new set of record in the same file
                ...
                if(!$upload->where('UNIQUE_KEY', '=', $record['UNIQUE_KEY'])->exists()){
                    ...
                    // $upload->GTIN = $record['GTIN'] ? $record['GTIN'] : NULL;
                    $upload->save();
                }else{
                    ...
                    $upload->update([
                        ...
                        // 'GTIN' => $record['GTIN'] ? $record['GTIN'] : NULL
                    ]);
                }
            }

            // Update process status into Completed
            $uploadedFiles = UploadedFiles::where('id',$this->file_id);
            $uploadedFiles->update(['status' => 2]);
        }
        catch(Exception $e) {
            $this->failed($e);
        }
    ...
    public function failed($exception)
    {
        ...
        // Update process status into Failed
        $uploadedFiles = UploadedFiles::where('id',$this->file_id);
        $uploadedFiles->update(['status' => 3]);
    }
}
